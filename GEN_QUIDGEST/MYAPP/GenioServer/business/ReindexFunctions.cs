...
namespace CSGenio.business
{
    public class ReindexFunctions
    {
        ...
        public void DeleteInvalidRows(CancellationToken cToken) {
            ...
            dm = sp.Execute(
                new SelectQuery()
                .Select(CSGenioAawayteam.FldCodteam)
                .From(CSGenioAawayteam.AreaAWAYTEAM)
                .Where(CriteriaSet.And().In(CSGenioAawayteam.FldZzstate, zzstateToRemove))
                );

            for (int i = 0; i < dm.NumRows; i++)
            {
                CSGenioAawayteam model = new CSGenioAawayteam(user);
