...
namespace GenioMVC.Controllers
{
	public partial class PlayersController : ControllerBase
	{
		...
		public PlayersController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL PNL CONTROLLER_NAVIGATION PLAYERS]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAplayers>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL PNL MANUAL_CONTROLLER PLAYERS]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Players row = new Models.Players(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("players");
			...
			try
			{
				...
				{
					case "F_PLAYERS__MATCHES__MATCHID":	// Field (DB)
						{
							var model = new F_players_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_players__matches__matchid(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_PLAYERS__MATCHES__MATCHID":	// Field (DB)
						values = new F_players_ViewModel(UserContext.Current).GetDependant_F_playersTableMatchesMatchid(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_players" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_players([FromBody]F_players_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "players",
				(primaryKey) => Models.Players.Find(primaryKey, UserContext.Current, "FF_PLAYERS"),
				(model) => formData.MapToModel(model as Models.Players)
			);
		}
		...
		{
			return base.GetDocumsTickets("PLAYERS", requestModel.FieldName, requestModel.KeyValue);
		}
