...
namespace GenioMVC.Controllers
{
	public partial class MatchesController : ControllerBase
	{
		...
		public MatchesController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL PNL CONTROLLER_NAVIGATION MATCHES]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAmatches>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL PNL MANUAL_CONTROLLER MATCHES]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Matches row = new Models.Matches(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("matches");
			...
			try
			{
				...
				{
					case "F_MATCHES__AWAYTEAM__TEAMID":	// Field (DB)
						{
							var model = new F_matches_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_matches__awayteam__teamid(qs);
							result = model.TableAwayteamTeamid;
						}
						break;
					case "F_MATCHES__TEAM__TEAMID":	// Field (DB)
						{
							var model = new F_matches_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_matches__team__teamid(qs);
							result = model.TableTeamTeamid;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_MATCHES__AWAYTEAM__TEAMID":	// Field (DB)
						values = new F_matches_ViewModel(UserContext.Current).GetDependant_F_matchesTableAwayteamTeamid(Selected);
						break;
					case "F_MATCHES__TEAM__TEAMID":	// Field (DB)
						values = new F_matches_ViewModel(UserContext.Current).GetDependant_F_matchesTableTeamTeamid(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_matches" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_matches([FromBody]F_matches_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "matches",
				(primaryKey) => Models.Matches.Find(primaryKey, UserContext.Current, "FF_MATCHES"),
				(model) => formData.MapToModel(model as Models.Matches)
			);
		}
		...
		{
			return base.GetDocumsTickets("MATCHES", requestModel.FieldName, requestModel.KeyValue);
		}
