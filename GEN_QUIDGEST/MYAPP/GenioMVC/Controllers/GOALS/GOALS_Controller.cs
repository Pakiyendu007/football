...
namespace GenioMVC.Controllers
{
	public partial class GoalsController : ControllerBase
	{
		...
		public GoalsController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL PNL CONTROLLER_NAVIGATION GOALS]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAgoals>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL PNL MANUAL_CONTROLLER GOALS]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Goals row = new Models.Goals(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("goals");
			...
			try
			{
				...
				{
					case "F_GOALS__MATCHES__MATCHID":	// Field (DB)
						{
							var model = new F_goals_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_goals__matches__matchid(qs);
							result = model.TableMatchesMatchid;
						}
						break;
					case "F_GOALS__PLAYERS__PLAYERID":	// Field (DB)
						{
							var model = new F_goals_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_goals__players__playerid(qs);
							result = model.TablePlayersPlayerid;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_GOALS__MATCHES__MATCHID":	// Field (DB)
						values = new F_goals_ViewModel(UserContext.Current).GetDependant_F_goalsTableMatchesMatchid(Selected);
						break;
					case "F_GOALS__PLAYERS__PLAYERID":	// Field (DB)
						values = new F_goals_ViewModel(UserContext.Current).GetDependant_F_goalsTablePlayersPlayerid(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_goals" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_goals([FromBody]F_goals_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "goals",
				(primaryKey) => Models.Goals.Find(primaryKey, UserContext.Current, "FF_GOALS"),
				(model) => formData.MapToModel(model as Models.Goals)
			);
		}
		...
		{
			return base.GetDocumsTickets("GOALS", requestModel.FieldName, requestModel.KeyValue);
		}
