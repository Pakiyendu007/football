<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-f-players-matches-matchid-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['F_PLAYERS__MATCHES__MATCHID_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FPlayersMatchesMatchidSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('F_PLAYERS__MATCHES__MATCHID_SeeMore', false),

				interfaceMetadata: {
					id: 'F_PLAYERS__MATCHES__MATCHID_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'F_PLAYERS__MATCHES__MATCHID_SeeMore',
					name: 'F_PLAYERS__MATCHES__MATCHID_SeeMore',
					controller: 'PLAYERS',
					action: 'F_PLAYERS_MatchesValMatchid'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-f-players-matches-matchid',
				dismissAction: this.close,
				returnElement: 'F_PLAYERS__MATCHES__MATCHID_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.MATCHES56954),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-f-players-matches-matchid')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'F_players_MatchesValMatchid',
							serverMode: true,
						...
						uuid: 'F_players_F_players_MatchesValMatchid',
