<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-f-matches-awayteam-teamid-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['F_MATCHES__AWAYTEAM__TEAMID_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FMatchesAwayteamTeamidSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('F_MATCHES__AWAYTEAM__TEAMID_SeeMore', false),

				interfaceMetadata: {
					id: 'F_MATCHES__AWAYTEAM__TEAMID_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'F_MATCHES__AWAYTEAM__TEAMID_SeeMore',
					name: 'F_MATCHES__AWAYTEAM__TEAMID_SeeMore',
					controller: 'MATCHES',
					action: 'F_MATCHES_AwayteamValTeamid'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-f-matches-awayteam-teamid',
				dismissAction: this.close,
				returnElement: 'F_MATCHES__AWAYTEAM__TEAMID_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.TEAM57329),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-f-matches-awayteam-teamid')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'F_matches_AwayteamValTeamid',
							serverMode: true,
							pkColumn: 'ValCodteam',
							tableAlias: 'AWAYTEAM',
							tableNamePlural: computed(() => this.Resources.TEAM57329),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
							...
							defaultSearchColumnName: 'ValTeamid',
							defaultSearchColumnNameOriginal: 'ValTeamid',
							defaultColumnSorting: {
								columnName: '',
								sortOrder: 'asc'
							}
						},
						globalEvents: ['changed-AWAYTEAM'],
						uuid: 'F_matches_F_matches_AwayteamValTeamid',
