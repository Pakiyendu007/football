<template>
	...
	<qtable 
		...
		<template #PNL="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'PNL' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					PNL : { active: true, Cod: "PNL", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.PNL.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.PNL.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.PNL.Description),
					name: "PNL",
					slot_name: "PNL",
					sort: false,
					visibility: vm.Modules.PNL.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
